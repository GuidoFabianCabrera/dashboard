          ...
          <template v-slot:top>
            <v-toolbar flat>
              ...
              <v-row>
                ...
                <v-col>
                  <v-text-field
                    v-model="filteredItem.dimension"
                    label="Dimension"
                  ></v-text-field>
                </v-col>
              </v-row>
...
export default {
    ...
    itemFiltered() {
      let cloneItems = this.items.filter(item => {
        if (
          ...
          item.dimension
            .toLowerCase()
            .includes(this.filteredItem.dimension.toLowerCase())
        )
          return item;
      });
  ...
  methods: {
    async initialize() {
      try {
        const data = await this.$repository.location.search();
        this.items = data;
      } catch (error) {
        console.log(error);
      }
    ...
    async deleteItemConfirm() {
      try {
        await this.$repository.location.deleteOne(
      ...
      if (this.editedIndex > -1) {
        try {
          const data = {
            ...
            dimension: this.editedItem.dimension,
            residents: this.items[this.editedIndex].residents,
            url: this.items[this.editedIndex].url
          };

          await this.$repository.location.updateOne(
            this.items[this.editedIndex]._id,
            data
          );
        ...
        try {
          ...
          await this.$repository.location.createOne(data);
          this.initialize();
          this.close();
        } catch (error) {
          console.log(error);
